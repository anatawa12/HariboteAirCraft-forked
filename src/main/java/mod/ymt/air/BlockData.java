...
public class BlockData {
	...
	public boolean equals(Object obj) {
		...
		if (block.equals(other.block) == false)
			return false;
	...
	public String toString() {
		return String.format("BlockData [block=%s(%s), metadata=%s, relPos=%s, absPos=%s]", block, block, metadata, relPos, absPos);
	}

	public void write(DataOutput output) throws IOException {
		int blocks = pack(Block.getIdFromBlock(block), metadata);
	...
	public static BlockData read(DataInput input, Coord3D base) throws IOException {
		...
		if (first == -1) {
			...
			return valueOf(unpackBlock(block), unpackMetadata(block), relPos, relPos.add(base));
		}
		else {
	...
	public static Block unpackBlock(int block) {
		return Block.getBlockById(block >>> 4);
	}
	...
	public static BlockData valueOf(Block block, int metadata, Coord3D relPos, Coord3D absPos) {
		if (block != null) {
